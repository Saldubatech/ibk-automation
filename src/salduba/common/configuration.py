...
#       msg2 = "Ensure that the HOME environment variable is set to a valid directory (folder)"
#       _logger.error(msg1 + '\n\t' + msg2)
...
"""
  ...
  def to_dict(cls) -> dict[str, Any]:
    meta = Meta()
    return {
      meta.app_id : cls.cervino.__dict__,
      "log_file_path": cls.log_file_path,
      "log_file_contents": cls.log_file_contents,
      meta.input_key : cls.input.__dict__,
      meta.output_key: cls.output.__dict__,
      meta.db_key: cls.db.__dict__,
      meta.tws_key: cls.tws.__dict__
    }

Y_TYPE = Union[dict[str, Any], list[Any], Tuple[Any], set[Any], Optional[bool], Optional[str], Optional[float], Optional[int]]
def deep_merge(original: Y_TYPE, defaults: Y_TYPE) -> Y_TYPE:
  if type(original) == dict and type(defaults) == dict:
    drs: dict[str, Any] = {}
    for k, v in defaults.items():
      if not k in original:
        drs[k] = v
      else:
        if v.type in [dict, list, tuple]:
          drs[k] = deep_merge(original[k], v)
        else:
          drs[k] = original[k]
    return drs
  elif type(original) == list and type(defaults) == dict:
    lrs = list(original)
    for d in defaults:
      if d not in original:
        lrs.append(d)
    return lrs
  elif type(original) == set and type(defaults) == set:
    return original.union(defaults)
  elif type(original) == tuple and type(defaults) == tuple:
    trs = list(original)
    for d in defaults:
      if d not in original:
        trs.append(d)
    ...
    return original if original else defaults
