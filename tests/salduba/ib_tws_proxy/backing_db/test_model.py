...
from salduba.ib_tws_proxy.backing_db.model import ContractRecordStatus, ContractRecord, contractCompanion
  ...
  def test_contract_record(self) -> None:
    ...
    with self.__local_db__() as db:
      with db.cursor() as crs_inserter:
        ...
        crs_inserter.execute(
          contractCompanion.value_inserter,
          probe_values
        )
      with db.cursor() as crs_reader:
        tbls = crs_reader.execute(
            "SELECT name FROM sqlite_master WHERE type='table';").fetchall()
        assert set([t[0].upper() for t in tbls]) == expected_tables
        records = crs_reader.execute(
          contractCompanion.selector+';'
        ).fetchall()
