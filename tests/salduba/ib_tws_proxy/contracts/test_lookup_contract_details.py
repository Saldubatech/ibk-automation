...
class TestLookupContractDetails(unittest.TestCase):
  ...
  def _postprocessor(self, contract: Contract, details: list[dict[str, ContractDetails]]) -> None:
    ...
    assert contract.exchange == details[0]['contractDetails'].contract.exchange,\
  ...
  def test_a_bunch(self):
    ...
    bunchProbe = [
      buildContract(r.Symbol, r.IBKType, r.Exchange, r.Currency)
      for r in bunch.itertuples()]
    print(f"###: Bunch({len(bunchProbe)})")
    for c in bunchProbe[0:10]:
      print(f"\t###{c}")
    underTest = LookupContractDetails(bunchProbe[0:10], self._postprocessor, self.host, self.port, self.appId, 60, logging.DEBUG, 0.1)
    underTest.activate()
