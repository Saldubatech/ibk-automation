...
class TestVerifyContracts(unittest.TestCase):
  ...
  def __init__(self, *args) -> None:
    ...
    self.db.configure()
    self.contract_repo, self.order_repo, self.movement_repo = TestVerifyContracts.repos(self.db)
  ...
  def test_on_empty_db(self) -> None:
    ...
    result = underTest.verify_contracts_for_csv_file(
      probe,
      output_file_path,
      True)
  ...
  def test_lookup_contracts(self) -> None:
    ...
    output_file_path = tmp_file.name
    _logger.info(f"Output file at: {output_file_path}")
    result = underTest.lookup_contracts(
      probe,
  ...
  def test_something_eld(self) -> None:
    pass
