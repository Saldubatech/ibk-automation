...
from salduba.ib_tws_proxy.domain.enumerations import Currency, Exchange, SecType, Country
from salduba.util.logging import init_logging
from salduba.util.tests import testRoot
from salduba.corvino.parse_input import split_ticker
...
class TestParseInput(unittest.TestCase):
    ...
    def test_read_excel(self) -> None:
      ...
      self.assertEqual(set([
        'ID ISIN', 'Nombre', 'Trade', 'Currency', 'Exchange', 'Exchange2', 'Symbol', 'Country', 'RawType', 'IbkType'
      ]), set(result.columns))
      self.assertEqual('2875 JP Equity', result.index[0])
      expect_in_0 = {'ID ISIN': 'JP3613000003',
    ...
    def test_read_full_excel(self) -> None:
      ...
      try:
        ...
        self.assertEqual("Exchange is empty for first row. Probably the formulas are not evaluated", str(exc))
    ...
    def test_read_csv(self) -> None:
      ...
      self.assertEqual(set([
        'Symbol', 'Nombre', 'Trade', 'Currency', 'Exchange', 'Exchange2', 'RawType', 'Country', 'IbkType',
      ]), set(result.columns))
      self.assertEqual('2222 JP Equity', result.index[0])
      expect_in_0 = {'Nombre': 'Kotobuki Spirits Co Ltd',
                     'Trade': -2400,
                     'Currency': Currency.JPY,
                     'Exchange': Exchange.TSEJ,
                     'Exchange2': Exchange.NONE,
